...
BASESUBJ="/C=CN/ST=GuangDong/L=ShenZhen/O=Leegoogol Ltd/OU=Software"
...
create_ca() {
	...
	openssl req -config openssl_me.cnf -key ${RootCA_DIR}/private/ca.key.pem -new -x509 -days 7300 -sha256 -extensions v3_ca -out ${RootCA_DIR}/certs/ca.cert.pem -subj "${BASESUBJ}/CN=Root CA/"
	...
	openssl req -config openssl_me.cnf -new -sha256 -key ${IntermediateCA_DIR}/private/intermediate.key.pem -out ${IntermediateCA_DIR}/csr/intermediate.csr.pem -subj "${BASESUBJ}/CN=Inter CA/"

	# 用根证书签名中间证书请求,生成中间证书
	echo "+-+-+-+- Create Intermediate CA Certificate -+-+-+-+"
	openssl ca -config openssl_me.cnf -name CA_root -extensions v3_intermediate_ca -days 3650 -notext -md sha256 -in ${IntermediateCA_DIR}/csr/intermediate.csr.pem -out ${IntermediateCA_DIR}/certs/intermediate.cert.pem
	...
	# 创建服务器证书签名请求
	openssl req -config openssl_me.cnf -key ${CA_DIR}/www.leegoogol.com.key.pem -new -sha256 -out ${CA_DIR}/www.leegoogol.com.csr.pem -subj "${BASESUBJ}/CN=www.leegoogol.com/"

	# 使用中间证书签名服务器证书签名请求
	openssl ca -config openssl_me.cnf -name CA_inter -extensions server_cert -days 375 -notext -md sha256 -in ${CA_DIR}/www.leegoogol.com.csr.pem -out ${CA_DIR}/www.leegoogol.com.cert.pem
